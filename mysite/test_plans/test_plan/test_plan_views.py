...
def create_plan(cursor, request):
    ...
    if None not in approve_plan(category, name, type, details) and any(approve_plan(category, name, type, details)):
        return JsonResponse({'error': '[ Name + Plan type ] already exists.'})
...
def edit_plan(cursor, request):
    ...
    cursor.execute(
        '''
        SELECT id, category, plan_details
        FROM test_plan
        WHERE id = %s AND ((plan_details ->> 'editor') ::jsonb @> to_jsonb(ARRAY[%s]) OR ((plan_details ->> 'creator')::integer = %s));
        ''',
        (plan_id, user_views.mail_to_userid(user_id), user_views.mail_to_userid(user_id))
    )
    result = cursor.fetchone()
    if not result:
       return JsonResponse({'error': 'Insufficient permissions'}) 
    else:
        [id, category, plan_details] = result
        if None not in approve_plan(category, name, type, details, plan_id) and any(approve_plan(category, name, type, details, plan_id)):
            return JsonResponse({'error': '[Name + Plan type] or version already exists.'})
        item_order = json.loads(plan_details)["item_order"]
        if not any(approve_plan(category, name, type, details, plan_id)):
            cursor.execute(
                '''
                INSERT INTO test_plan_record (plan_details, editor, plan_status, update_time)
                VALUES (%s, %s, %s, %s)
        ...
        plan_info = {
            ...
            "editor": json.loads(plan_details)["editor"],
            "reviewer": json.loads(plan_details)["reviewer"],
            "select": json.loads(plan_details)["select"] if any(approve_plan(category, name, type, details, plan_id)) else plan_record_id,
        ...
        cursor.execute(
            '''
            UPDATE test_plan
            SET type = %s, plan_details = %s
            WHERE id = %s;
            ''',
            (type, json.dumps(plan_info), plan_id)
        )
        return JsonResponse({'finaldata': 'Successful'})
...
# Boolean[Name+type, version]    
@with_db_connection
def approve_plan(cursor, category, name, type, details, plan_list_id=None):
    if plan_list_id:
        cursor.execute(
            '''
            SELECT * 
            ...
            AND EXISTS (
                SELECT 1
                FROM test_plan_record AS tpr
                WHERE tpr.plan_details = %s
                AND tpr.id IN (
                    SELECT jsonb_array_elements_text(tp.plan_details->'item_order')::int
                )
            );
            ''',
            (plan_list_id, json.dumps(details))
        )
        if cursor.fetchone():
            cursor.execute(
                '''
                SELECT * FROM test_plan
                WHERE category = %s AND type =%s AND plan_details ->> 'name' = %s
                ''',
                (category, type, name)
            )
            if cursor.fetchall():
            ...
            cursor.execute(
                '''
                SELECT * FROM test_plan
                WHERE category = %s AND type =%s AND plan_details ->> 'name' = %s AND id != %s
                ''',
                (category, type, name, plan_list_id)
            )
            if cursor.fetchall():
                return [True, False]
            else:
